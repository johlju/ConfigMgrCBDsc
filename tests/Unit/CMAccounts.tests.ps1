...
BeforeAll {
    ...
    $helperModuleName = 'DscResource.UnitTestHelper'

    Remove-Module -Name $helperModuleName -Force -ErrorAction 'SilentlyContinue'

    # This could be moved to an actual TestHelper module file that is imported instead.
    New-Module -Name $helperModuleName -ScriptBlock {
        function Initialize-TestEnvironmentWrapper
        {
            param
            (
...
Describe "$dscResourceName\Get-TargetResource" -Tag 'Get' {
    BeforeAll {
        # This is also used in the Describe-block description.
        $dscResourceName = 'DSC_CMAccounts'

        $testEnvironment = Initialize-TestEnvironmentWrapper -DscResourceName $dscResourceName
        ...
        Mock -CommandName Import-ConfigMgrPowerShellModule -ModuleName 'ConfigMgrCBDsc.ResourceHelper'
        Mock -CommandName Set-Location

        # TODO: Add correct mock for Get-CimInstance
